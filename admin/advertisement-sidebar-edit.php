...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
    	...
    	if($path == '') {
			...
			$statement = $pdo->prepare("UPDATE tbl_advertisement_sidebar SET adv_url=? WHERE adv_id=?");
        	...
        	// updating into the database
			$statement = $pdo->prepare("UPDATE tbl_advertisement_sidebar SET adv_photo=?, adv_url=?, adv_location=? WHERE adv_id=?");
...
if(!isset($_REQUEST['id'])) {
	...
	$statement = $pdo->prepare("SELECT * FROM tbl_advertisement_sidebar WHERE adv_id=?");
