...
if(isset($_POST['form_page'])) {
    ...
    if($valid == 1) {
        ...
        $statement = $pdo->prepare("INSERT INTO tbl_menu (menu_type,page_id,category_id,menu_name,menu_url,menu_order,menu_parent) VALUES (?,?,?,?,?,?,?)");
...
if(isset($_POST['form_category'])) {
    ...
    if($valid == 1) {
        ...
        // saving into the database
        $statement = $pdo->prepare("INSERT INTO tbl_menu (menu_type,page_id,category_id,menu_name,menu_url,menu_order,menu_parent) VALUES (?,?,?,?,?,?,?)");
        $statement->execute(array('Category','',$_POST['category_id'],'','',$_POST['menu_order'],$_POST['menu_parent']));
...
if(isset($_POST['form_other'])) {
    ...
    if($valid == 1) {
        ...
        // saving into the database
        $statement = $pdo->prepare("INSERT INTO tbl_menu (menu_type,page_id,category_id,menu_name,menu_url,menu_order,menu_parent) VALUES (?,?,?,?,?,?,?)");
        $statement->execute(array('Other','','',$_POST['menu_name'],$_POST['menu_url'],$_POST['menu_order'],$_POST['menu_parent']));
		...
		<div class="col-md-12">
			...
			<div class="nav-tabs-custom">
				<ul class="nav nav-tabs">
					...
					<li><a href="#tab_2" data-toggle="tab">Category as Menu</a></li>
							...
							<div class="box box-info">
								<div class="box-body">
									...
									<div class="form-group">
										<label for="" class="col-sm-2 control-label"></label>
										<div class="col-sm-6">
											<button type="submit" class="btn btn-success pull-left" name="form_category">Submit</button>
										</div>
									</div>
								</div>
							</div>
